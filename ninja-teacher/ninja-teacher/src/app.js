...
async function startServer() {
  try {
    ...
    // ⚠️ alter: true خطير مع SQLite — بيعيد بناء الجدول كامل (نسخ كل
    // الصفوف لجدول مؤقت)، وأي تعارض بسيط في الداتا الموجودة (مثلاً
    // قيمة NULL في عمود unique) بيكسر السيرفر بالكامل عند الإقلاع.
    //
    // بقى اختياري عن طريق .env:
    //   DB_AUTO_ALTER=true   → يحاول يضيف/يعدل الأعمدة تلقائيًا (احذر)
    //   (الافتراضي/غير موجود) → ينشئ الجداول الناقصة بس، من غير ما يلمس
    //                            الجداول والأعمدة الموجودة فعلاً (آمن)
    const shouldAlter = process.env.DB_AUTO_ALTER === "true";
    ...
    try {
      ...
      if (shouldAlter) {
        // لو الـ alter فشل، منرميش السيرفر كله — نرجع لوضع آمن
        console.error("⚠️  sync({alter:true}) failed, falling back to safe sync():");
        console.error("   →", syncErr.message);
        await sequelize.sync();
        console.log("✅ Models synced (fallback, no alter)");
      } else {
        throw syncErr;
      }
    }
